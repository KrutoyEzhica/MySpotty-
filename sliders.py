...
class Slider:
    def __init__(self, canvas, x, y, width, song, bounds=None):
        ...
        with open("themes.json", "r", encoding="utf-8") as f:
            main = json.load(f)
            main = main[main[0]["main"]]["slider"]
        self.line = canvas.create_line(
            # width=10
            bounds[0] + width // 2, bounds[1] + width // 2, bounds[2] - width // 2, bounds[3] - width // 2, width=5, fill=main[0], tags="slider")
        ...
        self.line1 = canvas.create_line(
            # width=10
            bounds[0] + width // 2, bounds[1] + width // 2, bounds[0] + width // 2, bounds[3] - width // 2, width=8, fill=main[1], tags="slider")
        canvas.tag_bind("slider", "<Button-1>", lambda event: self.song.rewind(
            int(self.song.duration * ((event.x - self.bounds[0]) / 400))))
        
        self.obj = canvas.create_oval(
            x, y, x + width, y + width, fill=main[2], width=0)
    ...
    def song_move(self, song_prec):
        if not self.isdrag:
            self.canvas.coords(self.obj, song_prec, 440,
                               song_prec + self.width, 460)
        self.canvas.coords(
            self.line1, self.bounds[0] + self.width // 2, self.bounds[1] +
            self.width // 2, song_prec +
            self.width, self.bounds[3] - self.width // 2
        )
    
    def recolor(self, colors):
        self.canvas.itemconfig(self.line, fill=colors[0])
        self.canvas.itemconfig(self.line1, fill=colors[1])
